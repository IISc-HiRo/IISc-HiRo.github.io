---
layout: main
title: People
permalink: /people/
---

<div class="people-section">
  <h1>People</h1>

  <!-- Group Photos Carousel -->
  <div class="group-carousel" style="max-width: 900px; margin: 2rem auto;">
    <div id="groupCarousel" class="carousel slide" data-bs-ride="carousel">
      <div class="carousel-indicators">
        <button type="button" data-bs-target="#groupCarousel" data-bs-slide-to="0" class="active" aria-current="true" aria-label="Group 2026"></button>
        <button type="button" data-bs-target="#groupCarousel" data-bs-slide-to="1" aria-label="Group 2025"></button>
      </div>
      <div class="carousel-inner" style="height: 450px;">
        <div class="carousel-item active">
          <img src="/assets/img/people/group_1_26.jpeg" class="d-block w-100" alt="HiRo Lab Group 2026" style="border-radius: 8px; height: 450px; object-fit: cover; object-position: center;">
        </div>
        <div class="carousel-item">
          <img src="/assets/img/people/group_8_25.jpg" class="d-block w-100" alt="HiRo Lab Group 2025" style="border-radius: 8px; height: 450px; object-fit: cover; object-position: center;">
        </div>
      </div>
      <button class="carousel-control-prev" type="button" data-bs-target="#groupCarousel" data-bs-slide="prev">
        <span class="carousel-control-prev-icon" aria-hidden="true"></span>
        <span class="visually-hidden">Previous</span>
      </button>
      <button class="carousel-control-next" type="button" data-bs-target="#groupCarousel" data-bs-slide="next">
        <span class="carousel-control-next-icon" aria-hidden="true"></span>
        <span class="visually-hidden">Next</span>
      </button>
    </div>
  </div>

  <h2>Faculty</h2>
  <div class="people-grid">
    <div class="person-card">
      <a href="/people/drraviprakash/" class="person-image-link">
        <img src="/assets/img/people/ravi_prakash.jpg" alt="Dr. Ravi Prakash"/>
      </a>
      <div class="person-links">
        <a href="/people/drraviprakash/" title="Personal Page"><i class="fas fa-globe"></i></a>
        <a href="https://scholar.google.com/citations?user=mV2CW4QAAAAJ&hl=en" target="_blank" title="Google Scholar"><i class="fas fa-graduation-cap"></i></a>
        <a href="https://www.linkedin.com/in/ravi-prakash-a93ab9a8/" target="_blank" title="LinkedIn"><i class="fab fa-linkedin"></i></a>
      </div>
      <span class="name">Prof. Ravi Prakash</span>
      <span class="role">Principal Investigator</span>
    </div>
  </div>

  <h2>PhD Students</h2>
  <div class="people-grid">
    <div class="person-card">
      <a href="#" target="_blank" class="person-image-link">
        <img src="/assets/img/people/basu_hela.jpg" alt="Basu Hela"/>
      </a>
      <div class="person-links">
        <a href="#" target="_blank" title="Personal Page"><i class="fas fa-globe"></i></a>
        <a href="#" target="_blank" title="Google Scholar"><i class="fas fa-graduation-cap"></i></a>
        <a href="https://www.linkedin.com/in/basu-hela-a51533159" target="_blank" title="LinkedIn"><i class="fab fa-linkedin"></i></a>
      </div>
      <span class="name">Basu Hela</span>
      <span class="role">Foundational Models for Robotics</span>
    </div>
    <div class="person-card">
      <a href="https://varunibuereddy.github.io/" target="_blank" class="person-image-link">
        <img src="/assets/img/people/varuni_buereddy.jpeg" alt="Varuni Buereddy"/>
      </a>
      <div class="person-links">
        <a href="https://varunibuereddy.github.io/" target="_blank" title="Personal Page"><i class="fas fa-globe"></i></a>
        <a href="https://scholar.google.com/citations?hl=en&user=nTCygfMAAAAJ" target="_blank" title="Google Scholar"><i class="fas fa-graduation-cap"></i></a>
        <a href="https://www.linkedin.com/in/varunibuereddy/" target="_blank" title="LinkedIn"><i class="fab fa-linkedin"></i></a>
      </div>
      <span class="name">Varuni Buereddy</span>
      <span class="role">Safety Critical Control, Formal Methods for Imitation Learning</span>
    </div>
    <div class="person-card">
      <a href="https://sasidharalavala.work/" target="_blank" class="person-image-link">
        <img src="/assets/img/people/sasidhar_alavala.JPG" alt="Sasidhar Alavala"/>
      </a>
      <div class="person-links">
        <a href="https://sasidharalavala.work/" target="_blank" title="Personal Page"><i class="fas fa-globe"></i></a>
        <a href="https://scholar.google.com/citations?user=1PStmj8AAAAJ&hl=en" target="_blank" title="Google Scholar"><i class="fas fa-graduation-cap"></i></a>
        <a href="https://www.linkedin.com/in/sasidharalavala/" target="_blank" title="LinkedIn"><i class="fab fa-linkedin"></i></a>
      </div>
      <span class="name">Sasidhar Alavala</span>
      <span class="role">SLAM, World Models, Imitation Learning</span>
    </div>
    <div class="person-card">
      <a href="https://mritunjoyh.github.io/" target="_blank" class="person-image-link">
        <img src="/assets/img/people/mritunjoy_halder.jpeg" alt="Mritunjoy Halder"/>
      </a>
      <div class="person-links">
        <a href="https://mritunjoyh.github.io/" target="_blank" title="Personal Page"><i class="fas fa-globe"></i></a>
        <a href="https://scholar.google.com/citations?user=kl0CPgYAAAAJ&hl=en" target="_blank" title="Google Scholar"><i class="fas fa-graduation-cap"></i></a>
        <a href="https://www.linkedin.com/in/mritunjoyh/" target="_blank" title="LinkedIn"><i class="fab fa-linkedin"></i></a>
      </div>
      <span class="name">Mritunjoy Halder</span>
      <span class="role">Autonomous Space Robotics under Uncertainty</span>
    </div>
  </div>

  <h2>MTech(R) &amp; MTech Students</h2>
  <div class="people-grid">
    <div class="person-card">
      <a href="#" target="_blank" class="person-image-link">
        <img src="/assets/img/people/akhil_r_kurup.jpg" alt="Akhil R Kurup"/>
      </a>
      <div class="person-links">
        <a href="#" target="_blank" title="Personal Page"><i class="fas fa-globe"></i></a>
        <a href="#" target="_blank" title="Google Scholar"><i class="fas fa-graduation-cap"></i></a>
        <a href="https://www.linkedin.com/in/akhil-r-kurup" target="_blank" title="LinkedIn"><i class="fab fa-linkedin"></i></a>
      </div>
      <span class="name">Akhil R Kurup</span>
      <span class="role">MTech(R)</span>
    </div>
    <div class="person-card">
      <a href="https://mumukshtayal.github.io/" target="_blank" class="person-image-link">
        <img src="/assets/img/people/mumuksh_tayal.jpeg" alt="Mumuksh Tayal"/>
      </a>
      <div class="person-links">
        <a href="https://mumukshtayal.github.io/" target="_blank" title="Personal Page"><i class="fas fa-globe"></i></a>
        <a href="https://scholar.google.com/citations?user=4d4NEc8AAAAJ&hl=en" target="_blank" title="Google Scholar"><i class="fas fa-graduation-cap"></i></a>
        <a href="https://www.linkedin.com/in/mumuksh-tayal-9802aa1b9" target="_blank" title="LinkedIn"><i class="fab fa-linkedin"></i></a>
      </div>
      <span class="name">Mumuksh Tayal</span>
      <span class="role">MTech(R)</span>
    </div>
    <div class="person-card">
      <a href="https://ashrithsagar.github.io/" target="_blank" class="person-image-link">
        <img src="/assets/img/people/ashrith_yedlapalli.jpeg" alt="Ashrith Sagar Yedlapalli"/>
      </a>
      <div class="person-links">
        <a href="https://ashrithsagar.github.io/" target="_blank" title="Personal Page"><i class="fas fa-globe"></i></a>
        <a href="https://scholar.google.com/citations?user=io_P3R0AAAAJ&hl=en" target="_blank" title="Google Scholar"><i class="fas fa-graduation-cap"></i></a>
        <a href="https://www.linkedin.com/in/ashrith9sagar/" target="_blank" title="LinkedIn"><i class="fab fa-linkedin"></i></a>
      </div>
      <span class="name">Ashrith Sagar Yedlapalli</span>
      <span class="role">MTech</span>
    </div>
    <div class="person-card">
      <a href="#" target="_blank" class="person-image-link">
        <img src="/assets/img/people/shubham_puri_goswami.png" alt="Shubham Puri Goswami"/>
      </a>
      <div class="person-links">
        <a href="#" target="_blank" title="Personal Page"><i class="fas fa-globe"></i></a>
        <a href="#" target="_blank" title="Google Scholar"><i class="fas fa-graduation-cap"></i></a>
        <a href="https://www.linkedin.com/in/shubhampurigoswami" target="_blank" title="LinkedIn"><i class="fab fa-linkedin"></i></a>
      </div>
      <span class="name">Shubham Puri Goswami</span>
      <span class="role">MTech</span>
    </div>
    <div class="person-card">
      <a href="#" target="_blank" class="person-image-link">
        <img src="/assets/img/people/swapnil_joshi.jpeg" alt="Swapnil Joshi"/>
      </a>
      <div class="person-links">
        <a href="#" target="_blank" title="Personal Page"><i class="fas fa-globe"></i></a>
        <a href="#" target="_blank" title="Google Scholar"><i class="fas fa-graduation-cap"></i></a>
        <a href="https://www.linkedin.com/in/swapnil-joshi-059731357/" target="_blank" title="LinkedIn"><i class="fab fa-linkedin"></i></a>
      </div>
      <span class="name">Swapnil Joshi</span>
      <span class="role">MTech</span>
    </div>
    <div class="person-card">
      <a href="https://aryansingh27022003.github.io/Portfolio/" target="_blank" class="person-image-link">
        <img src="/assets/img/people/aryan_kumar_singh.png" alt="Aryan Kumar Singh"/>
      </a>
      <div class="person-links">
        <a href="https://aryansingh27022003.github.io/Portfolio/" target="_blank" title="Personal Page"><i class="fas fa-globe"></i></a>
        <a href="#" target="_blank" title="Google Scholar"><i class="fas fa-graduation-cap"></i></a>
        <a href="https://www.linkedin.com/in/aryan-kumar-singh-334865227/" target="_blank" title="LinkedIn"><i class="fab fa-linkedin"></i></a>
      </div>
      <span class="name">Aryan Kumar Singh</span>
      <span class="role">MTech</span>
    </div>
  </div>

  <h2>Research Staff</h2>
  <div class="people-grid">
    <div class="person-card">
      <a href="#" target="_blank" class="person-image-link">
        <img src="/assets/img/people/manthan_goyal.jpeg" alt="Manthan Goyal"/>
      </a>
      <div class="person-links">
        <a href="#" target="_blank" title="Personal Page"><i class="fas fa-globe"></i></a>
        <a href="#" target="_blank" title="Google Scholar"><i class="fas fa-graduation-cap"></i></a>
        <a href="https://www.linkedin.com/in/manthan-goyal-00a4601b7" target="_blank" title="LinkedIn"><i class="fab fa-linkedin"></i></a>
      </div>
      <span class="name">Manthan Goyal</span>
      <span class="role">Research Associate</span>
    </div>
  </div>

  <h2>Robots</h2>
  <div class="people-grid">
    <div class="person-card">
      <a href="https://franka.de/franka-research-3" target="_blank" class="person-image-link">
        <img src="/assets/img/people/franka_research_3.jpg" alt="Franka Research 3"/>
      </a>
      <span class="name">Franka Research 3 (&times;5)</span>
      <div class="person-links">
        <a href="https://franka.de/franka-research-3" target="_blank" title="Website"><i class="fas fa-globe"></i></a>
      </div>
    </div>
    <div class="person-card">
      <a href="https://global.agilex.ai/products/nero" target="_blank" class="person-image-link">
        <img src="/assets/img/people/agilex_nero.webp" alt="AgileX NERO"/>
      </a>
      <span class="name">AgileX NERO (&times;4)</span>
      <div class="person-links">
        <a href="https://global.agilex.ai/products/nero" target="_blank" title="Website"><i class="fas fa-globe"></i></a>
      </div>
    </div>
    <div class="person-card">
      <a href="https://global.agilex.ai/products/piper-x" target="_blank" class="person-image-link">
        <img src="/assets/img/people/agilex_piper_x.webp" alt="AgileX PiPER-X"/>
      </a>
      <span class="name">AgileX PiPER-X (&times;1)</span>
      <div class="person-links">
        <a href="https://global.agilex.ai/products/piper-x" target="_blank" title="Website"><i class="fas fa-globe"></i></a>
      </div>
    </div>
    <div class="person-card">
      <a href="https://global.agilex.ai/products/cobot-s-kit" target="_blank" class="person-image-link">
        <img src="/assets/img/people/agilex_cobot_s.webp" alt="AgileX COBOT S"/>
      </a>
      <span class="name">AgileX COBOT S (&times;1)</span>
      <div class="person-links">
        <a href="https://global.agilex.ai/products/cobot-s-kit" target="_blank" title="Website"><i class="fas fa-globe"></i></a>
      </div>
    </div>
    <div class="person-card">
      <a href="https://clearpathrobotics.com/jackal-small-unmanned-ground-vehicle/" target="_blank" class="person-image-link">
        <img src="/assets/img/people/jackal_ugv.png" alt="Clearpath Jackal UGV"/>
      </a>
      <span class="name">Jackal UGV (&times;1)</span>
      <div class="person-links">
        <a href="https://clearpathrobotics.com/jackal-small-unmanned-ground-vehicle/" target="_blank" title="Website"><i class="fas fa-globe"></i></a>
      </div>
    </div>
  </div>

  <h2>Past Members</h2>
  <h3>Research Staff</h3>
  <ul class="past-members">
    <li><a href="https://in.linkedin.com/in/anurag-maurya-617287215" target="_blank">Anurag Maurya</a> (2024&ndash;25)</li>
    <li><a href="https://www.linkedin.com/in/shr-eyas/" target="_blank">Shreyas Kumar</a> (2025&ndash;26) <span class="now">(Now: Founding Engineer at DexSent Robotics)</span></li>
  </ul>
</div>
